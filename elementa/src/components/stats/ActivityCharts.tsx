...
import { accuracyPerDay, learnedCumulative, questionsPerDay, sum } from './stats-data';
...
/** Progreso semanal: preguntas por día en los últimos 7 días, con la meta diaria como referencia. */
export function WeeklyChart({ state, today }: ChartProps) {
  ...
  return (
    <ChartCard
      ...
      summary={
        total > 0
          ? `${count(total, 'respuesta', 'respuestas')} en 7 días · meta cumplida ${goalDays} ${pluralize(goalDays, 'día', 'días')}`
          : `Tu meta: ${goal} al día`
      }
    ...
    >
      <ColumnChart
        data={data}
        ariaLabel={`Respuestas por día en los últimos 7 días. Total: ${total}. Hoy: ${values[values.length - 1]}. Mejor día: ${best}.`}
        describe={(v) => count(v, 'respuesta', 'respuestas')}
        reference={{ value: goal, label: `Meta ${goal}` }}
      />
      <ChartTable
        caption="Respuestas (preguntas y flashcards) por día, últimos 7 días"
        headers={['Día', 'Respuestas']}
...
/** Precisión diaria de los últimos 14 días (los días sin actividad quedan como huecos). */
export function AccuracyChart({ state, today }: ChartProps) {
  ...
  const totals = keys.reduce(
    (acc, k) => {
      ...
      return day ? { q: acc.q + day.questions, c: acc.c + Math.min(day.correct, day.questions) } : acc;
    },
    { q: 0, c: 0 },
  );
  const activeDays = values.filter((v) => v !== null).length;
  const average = totals.q > 0 ? Math.round((100 * totals.c) / totals.q) : null;
  ...
  return (
    ...
    >
      <LineChart
        ...
        ariaLabel={
          average !== null
            ...
            : 'Precisión diaria de las últimas 2 semanas: sin actividad.'
        }
        describe={(v) => `${v}%`}
        yMax={100}
        formatTick={(v) => `${v}%`}
      />
      <ChartTable
        caption="Precisión por día, últimas 2 semanas"
        headers={['Día', 'Precisión']}
        rows={data.map((d) => [d.label, d.value !== null ? `${d.value}%` : 'Sin actividad'] as const)}
      />
    </ChartCard>
  );
}
...
/** Respuestas por día en los últimos 30 días. */
export function QuestionsChart({ state, today }: ChartProps) {
  ...
  return (
    <ChartCard
      ...
      summary={
        total > 0
          ? `${count(total, 'respuesta', 'respuestas')} en 30 días · ${activeDays} ${pluralize(activeDays, 'día activo', 'días activos')}`
          : 'Últimos 30 días (preguntas y flashcards)'
      }
      emptyText={total === 0 ? 'Aquí verás tu constancia día a día.' : null}
    >
      <ColumnChart
        data={data}
        ariaLabel={`Respuestas por día en los últimos 30 días. Total: ${total}. Días activos: ${activeDays}. Mejor día: ${best}.`}
        describe={(v) => count(v, 'respuesta', 'respuestas')}
        valueLabels="key"
        grid
      />
      <ChartTable
        caption="Respuestas (preguntas y flashcards) por día, últimos 30 días"
        headers={['Día', 'Respuestas']}
        rows={data.map((d) => [d.label, String(d.value)] as const)}
      />
    </ChartCard>
