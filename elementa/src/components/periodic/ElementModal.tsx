...
export function ElementModal({ atomicNumber, onClose, onNavigate }: ElementModalProps) {
  ...
  return (
    <Modal open={valid} onClose={onClose} title={element?.name ?? 'Elemento'} hideTitle size="lg">
      <div ref={topRef} aria-hidden className="-mt-3 h-0" />
      {element && <ElementDetail atomicNumber={element.atomicNumber} onNavigate={navigate} titleAs="p" />}
    </Modal>
  );
}
